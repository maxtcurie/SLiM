...
#this script is optimized version of orginal DispersionRelationDeterminantFullConductivityZeff.py
#using cython to accelectrate the calculation 
...
cpdef parity_finder_short_cython(zgrid,f,name,plot,report): #this function is for local linear simulation with short range of z 
    cdef float zmin
    cdef float zmax
    cdef int idx
    cdef str parity1
    ...
    #truncation
    if abs(zmin) > abs(zmax): #take -abs(zmax) to abs(zmax)
        idx=np.argmin(abs(zgrid+zmax))
        new_z=zgrid[idx:-1]
        f=f[idx:-1]
    elif abs(zmin) < abs(zmax): #take -abs(zmin) to abs(zmin)
        idx=np.argmin(abs(zgrid+zmin))
        new_z=zgrid[0:idx]
        f=f[0:idx]
    else:
        new_z=zgrid
    ...
    parity[0]=np.sum(abs(f-f2))
    #odd parity sum(f(x)+f(-x)) = 0 if odd 
    parity[1]=np.sum(abs(f+f2))
    ...
    if plot==True:
        ...
        plt.axvline(x=0, label='Center of the symmetry', color="red")
